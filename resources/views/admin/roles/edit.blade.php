...
<div
    x-data="{
        ...
        selectedPermissions: @js($role->permissions->pluck('name')->values()),
...
>
    ...
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-3xl font-black text-gray-900 tracking-tight">Edit Role: <span class="text-[#D32F2F]">{{ $role->name }}</span></h2>
        </div>
    ...
    <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="flex flex-col gap-6">
        @csrf
        @method('PUT')
