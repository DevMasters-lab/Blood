...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-white rounded-[3rem] p-10 shadow-sm border border-gray-100 flex flex-col lg:flex-row items-center justify-between">
        <div class="flex items-center">
            ...
            <div class="ml-8">
                <h2 class="text-4xl font-black text-[#1A1C1E] tracking-tight">{{ __('ui.system_authority') }}</h2>
                <div class="flex items-center mt-2">
                    <span class="relative flex h-3 w-3 mr-3">
                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                        <span class="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span>
                    </span>
                    <p class="text-xs font-black text-gray-400 uppercase tracking-[0.3em]">{{ __('ui.administrator_name', ['name' => auth()->user()->name]) }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-50 group hover:shadow-xl transition-all duration-500">
            <div class="flex justify-between items-start mb-6">
                ...
                <span class="text-[10px] font-black text-gray-300 uppercase tracking-widest">{{ __('ui.database') }}</span>
            </div>
            <p class="text-4xl font-black text-[#1A1C1E]">{{ $totalUsers }}</p>
            <p class="text-xs text-gray-400 mt-2 font-bold uppercase tracking-wider">{{ __('ui.registered_users') }}</p>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-50 group hover:shadow-xl transition-all duration-500">
            <div class="flex justify-between items-start mb-6">
                ...
                <span class="text-[10px] font-black text-gray-300 uppercase tracking-widest">{{ __('ui.live') }}</span>
            </div>
            <p class="text-4xl font-black text-[#D32F2F]">{{ $pendingRequests }}</p>
            <p class="text-xs text-gray-400 mt-2 font-bold uppercase tracking-wider">{{ __('ui.active_requests') }}</p>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-50 group hover:shadow-xl transition-all duration-500">
            <div class="flex justify-between items-start mb-6">
                ...
                <span class="text-[10px] font-black text-gray-300 uppercase tracking-widest">{{ __('ui.pending') }}</span>
            </div>
            <p class="text-4xl font-black text-gray-800">{{ $pendingDonations }}</p>
            <p class="text-xs text-gray-400 mt-2 font-bold uppercase tracking-wider">{{ __('ui.to_verify') }}</p>
        </div>
        ...
        <div class="bg-[#1A1C1E] p-8 rounded-[2.5rem] shadow-2xl shadow-gray-900/20 text-white relative overflow-hidden group">
            <div class="relative z-10 h-full flex flex-col justify-between">
                <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-50">{{ __('ui.verified_success') }}</p>
    ...
    <div class="bg-[#D32F2F] rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-red-900/20">
        <div class="relative z-10 flex flex-col lg:flex-row items-center">
            ...
            <div class="flex-1 text-center lg:text-left">
                <h3 class="text-2xl font-black tracking-tight mb-2">{{ __('ui.security_overview') }}</h3>
                <p class="text-white/80 text-sm font-bold tracking-wide">{{ __('ui.security_overview_desc') }}</p>
            </div>
