...
<div class="space-y-6 animate-fade-in px-8 py-8">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data" class="p-8 md:p-10">
                @csrf
                @method('PUT')
                ...
                <div class="mb-10 flex items-center bg-gray-50 p-6 rounded-2xl border border-gray-100">
                    <div class="mr-6 relative group">
                        @if($user->avatar)
                            <img src="{{ asset('storage/' . $user->avatar) }}" class="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md" alt="Admin avatar">
                        @else
